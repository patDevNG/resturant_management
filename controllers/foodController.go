...
func GetFood() gin.HandlerFunc{
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil{
			c.JSON(http.StatusInternalServerError, gin.H{"error":"An error occurred while fetching food"})
		}
...
func CreateFood() gin.HandlerFunc{
	return func(c *gin.Context) {
		...
		foundFood := foodCollection.FindOne(ctx, bson.M{"name": food.Name}).Decode((&food))
		defer cancel()
		if foundFood != nil {
			msg := fmt.Sprintf("Food already exist")
...
func UpdateFood() gin.HandlerFunc{
	return func(c *gin.Context) {
		...
		if food.Menu_id != nil {
			...
			if err != nil {
				msg:= fmt.Sprintf("Invalid menu_id")
				c.JSON(http.StatusBadRequest, gin.H{"error": msg})
				return
			}
